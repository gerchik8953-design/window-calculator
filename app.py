...
def is_contact_request(text):
    """Проверяет, хочет ли клиент получить контакты (телефон, адрес, сайт, ВК, график)"""
    text_lower = text.lower()
    keywords = [
        'телефон', 'номер', 'позвонить', 'созвонить',
        'контакт', 'связаться', 'связь',
        'адрес', 'где находитесь', 'находитесь', 'офис', 'приехать',
        'сайт', 'вк', 'написать', 'график', 'режим работы'
    ]
...
- Используй контакты компании только если их явно запрашивают (телефон, адрес, сайт, ВК, график).
"""},
